...
namespace WPFCalcZielinski.Model
{
    public class Calculator : ICalculator
    {
        ...
        {
            if(!ValidateCommand(commandParam))
            {
                throw new CalculatorInvalidCommandException();
            }
            if (Error)
            {
                Reset();
        ...
        private bool ValidateCommand(string command)
        {
            ...
            if (!DecimalPointEnabled && command == ".")
            {
                return false;
            }
            int commandAsInt;
            return 
                int.TryParse(command, out commandAsInt)
                || (command == ".") 
                || (operationDic[command] != null);
        }
    }
    ...
    public class CalculatorInvalidCommandException : Exception
    {
